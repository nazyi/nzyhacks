...
categories: [modul]
layout: post
description: "Cisco Ethical Hacker kursunun ikinci bölümüdür. Sızma testi sürecinde planlama ve kapsam belirleme aşamalarının neden kritik olduğunu vurgular. Aynı zamanda regülasyonlara uyum, yasal sorumluluklar ve etik ilkeler çerçevesinde nasıl hareket edilmesi gerektiğini detaylandırır."
...
| Özellik | WADL | WSDL |
|---|---|---|
| Açılımı | Web Application Description Language | Web Services Description Language |
| Kapsam | RESTful web servisler için | SOAP tabanlı web servisler için |
| Veri formatı | XML | XML |
| Protokol | HTTP (REST API) | SOAP (Simple Object Access Protocol) |
| Kullanım tarzı | Hafif, URL tabanlı servisleri tanımlar | Daha karmaşık, işlem odaklı (RPC gibi) |
| Geliştirici dostu | Daha modern ve anlaşılır (REST mimarisiyle uyumlu) | Daha eski stil, katı kurallara sahip |
| Araç desteği | Postman, Curl, modern API araçları | SoapUI gibi SOAP odaklı araçlarla daha uyumlu |
...
{% include tf-quiz.html id="tf-module2-tr" items=site.data.tf_module2.items true_label="✓ Doğru" false_label="✗ Yanlış" correct_label="Doğru bildin!" wrong_label="Yanlış tahmin, bu ifade aslında" true_word="doğru" false_word="yanlış" %}
